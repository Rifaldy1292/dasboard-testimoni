...
module.exports = class MachineWebsocket {
  ...
  /**
   * Handles monthly percentage calculation and response
   * 
   * @param {WebSocket} client - The WebSocket client instance
   * @param {string} date - The date string for the month
   * @returns {Promise<void>}
   */
  static async handleMonthlyPercentages(client, date) {
    const nowDate = new Date(date);
    ...
    try {
      ...
      if (monthlyError.message.includes("cutting time not found")) {
        return client.send(JSON.stringify({
          type: 'error',
          message: 'No cutting time configuration found for this month'
        }));
      }
      throw monthlyError;
    }
    ...
    const lastDayWithData = isCurrentMonth
      ? Math.min(currentDay, monthlyData.allDateInMonth.length)
      : monthlyData.allDateInMonth.length;

    // Extract and validate target data
  ...
  static async percentages(client, data) {
    try {
      ...
      if (data.monthly) {
        return await this.handleMonthlyPercentages(client, date);
      }
